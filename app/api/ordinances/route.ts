...
export async function GET(request: NextRequest) {
  ...
  const result = await OrdinanceService.list(filters, session)
...
export async function POST(request: NextRequest) {
  ...
  const parsed = ordinanceCreateSchema.safeParse(body)
  ...
  try {
    const ordinance = await OrdinanceService.create(
      { ...parsed.data, createdBy: session.id, updatedBy: session.id },
      request.headers.get('x-forwarded-for') ?? undefined,
      request.headers.get('user-agent') ?? undefined
    )
    return NextResponse.json({ data: ordinance, message: 'Ordinance created successfully.' }, { status: 201 })
  } catch (error: any) {
