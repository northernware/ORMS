...
import { signatureSchema } from '@/lib/validators/resolution.schema'
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const resolution = await ResolutionService.recordSignature(
      Number(id),
      parsed.data,
      session,
    ...
    return NextResponse.json({ data: resolution, message: "Mayor's signature recorded — requester notified." })
  } catch (e: any) {
